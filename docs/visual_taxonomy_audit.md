...
This note records the correction from a literal chemical-table layout to a functional taxonomy matrix.
...
The final poster figure uses periodic-table-style cells, but the grid coordinates are no longer chemical and no longer use risk as the physical row:

| Coordinate | Meaning |
|---|---|
| Row | Attack surface or operational stage |
| Column | Consistent attack family |
| Cell label | Reference ID, symbol, short name, risk, and family |
...
`scripts/generate_reporting_artifacts.py` writes `results/table_layout_audit.csv`, `.json`, and `.md`. Each row reports the technique, poisoning/evasion panel, printed risk label, surface row, family column, and a status value. The expected status for every row is `consistent`.
