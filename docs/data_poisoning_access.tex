\documentclass[6p,10pt,times]{ieeeaccess}
\usepackage{cite}
...
\author{\uppercase{Yulia Kumar}\authorrefmark{1,2},
\uppercase{Dov Kruger}\authorrefmark{2}, and
\uppercase{J. Jenny Li}\authorrefmark{1}}

\address[1]{Department of Computer Science and Technology, Kean University, Union, NJ 07083 USA (e-mail: ykumar@kean.edu; juli@kean.edu)}
\address[2]{Department of Electrical and Computer Engineering, Rutgers University, Piscataway, NJ 08854 USA (e-mail: Dov.Kruger@rutgers.edu)}

\tfootnote{This manuscript extends an unpublished FoodDDP draft and associated reproducible reporting artifacts.}

\markboth
{Kumar \headeretal: Data Poisoning Risk Matrix}
{Kumar \headeretal: Data Poisoning Risk Matrix}

\corresp{Corresponding author: Yulia Kumar (e-mail: ykumar@kean.edu).}
...
The contributions are: (i) a mechanism-and-risk taxonomy for poisoning and related attacks; (ii) implementations for the original poster methods plus ten additional shared methods, audited in \texttt{results/implementation\_coverage.*}; (iii) pilot transfer-learning food-image experiments that avoid in-memory dataset loading and weak from-scratch baselines; and (iv) reporting artifacts, tables, graphs, and a layout audit suitable for a journal manuscript and companion poster.
...
The reported food-image experiments use a PyTorch transfer-learning classifier rather than a from-scratch toy network. Table~\ref{tab:modelconfigs} summarizes the exact model configurations. The default backbone is MobileNetV3-Small with ImageNet pretrained weights~\cite{mobilenetv3}. MobileNetV3-Small is a lightweight convolutional network built from mobile inverted bottleneck blocks, depthwise separable convolutions, squeeze-and-excitation modules, and hard-swish/ReLU nonlinearities. In our wrapper, the original ImageNet classifier is removed. The frozen \texttt{features} stack maps each normalized RGB image to a 576-channel final convolutional representation; adaptive average pooling converts this tensor to a 576-dimensional embedding; and the trainable head is \(\mathrm{Dropout}(0.20)\) followed by a linear layer of size \(576\times C\), where \(C\) is the number of dataset classes.
...
This is explicitly a transfer-learning, or linear-probing, threat model. Let \(z=f_\phi(x)\) denote the MobileNetV3 feature vector and \(g_w(z)\) denote the newly initialized classifier head. When \(f_\phi\) is in \texttt{eval()} mode and all backbone parameters have \texttt{requires\_grad=False}, \(z\) is deterministic for a fixed image. Therefore, caching \(z\) and optimizing \(g_w\) is mathematically equivalent to repeatedly forwarding the same images through the frozen backbone and training only the head. We use this equivalence to make repeated poisoned retraining feasible on one workstation. The trade-off is scope: the experiments demonstrate poisoning of the linear-probe classifier and its decision boundary, not corruption of the frozen deep representation itself. In cached-feature runs, online spatial or brightness augmentation is no longer applied after caching; a full-budget journal run should either train without caching, or cache multiple augmented views per image before fitting the head. Query-intensive evasion analogs such as ZOO and Boundary/HopSkipJump are evaluated on a conditional subset controlled by \texttt{--eval-samples}; where the dataset permits, the journal protocol should use at least 100 clean-correct eligible images. The fish micro-batch cannot meet this threshold and is therefore labeled as a proof-of-concept.
...
\small
\begin{tabular}{p{0.16\textwidth}p{0.18\textwidth}p{0.26\textwidth}p{0.30\textwidth}}
\toprule
Dataset/run & Backbone & Feature representation & Trainable structure and notes \\
\midrule
Fungi & MobileNetV3-Small, ImageNet pretrained, frozen features & Final feature map from \texttt{features}; adaptive average pooling to 576 dimensions & \(\mathrm{Dropout}(0.20)\rightarrow\mathrm{Linear}(576,2)\); default \(160\times160\) input; trained for the pilot fungi rows with class weighting and light augmentation \\
...
The generated table-layout audit should contain only \texttt{consistent} statuses. The \texttt{fungi/}, \texttt{fish/}, \texttt{Datasets/}, and pretrained-weight cache are excluded from version control. For journal-scale results, rerun the same commands over the seed ranges in Table~\ref{tab:seeds}, aggregate mean \(\pm\) standard deviation, use \texttt{--eval-samples 100} for query-intensive stress tests where the clean-correct pool permits, and either disable frozen-feature caching or cache multiple augmented views per image when augmentation is part of the training claim.
