...
\begin{table}[t]
\centering
\caption{New seafood freshness dataset from \texttt{Datasets/}. Counts use all 400 unique image hashes after excluding 122 duplicate \texttt{Anchovy/working/} split copies from the 522 JPG files on disk. Labels are inferred from repeated Fresh/NonFresh leaf folders across Anchovy and Horse Mackerel.}
...
\texttt{seafood\_dataset\_audit.md} & Audit & SHA-256 duplicate audit showing that the seafood run uses all 400 unique images and excludes only 122 duplicate \texttt{working/} copies. \\
