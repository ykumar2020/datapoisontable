...
Table~\ref{tab:results} gives a compact at-a-glance comparison. Full CSV, JSON, Markdown, and additional plots are generated under \texttt{results/} and \texttt{figures/}. The file \texttt{results/table\_layout\_audit.*} verifies the panel assignment, expected risk row, mechanism column, risk label, and poisoning-versus-evasion separation for each visual cell.
...
\textbf{RQ1:} The risk matrix answers the organization question by making every coordinate auditable: the vertical coordinate is exactly R1, R2, R3, or R4; the horizontal coordinate is the mechanism family; and the panel determines whether a method is poisoning proper or an evasion/transfer analog. This removes arbitrary chemical-table gaps, prevents a cell labeled R1 from appearing in an R2 or R3 row, and separates related evasion methods from data-poisoning methods.
